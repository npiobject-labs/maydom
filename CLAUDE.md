...
- **Una plantilla de búsqueda sin verificar no se da por buena**: el sandbox no alcanza las tiendas (el proxy las bloquea), así que una URL inventada acaba en un 404 que descubre el usuario. Cada tienda guarda `dominio` y `verificada`; sin plantilla se busca `site:dominio` en un buscador general, que siempre responde, y el botón «no funciona» de cada resultado pasa esa tienda a ese modo sin tocar código. Comprobado roto el 21-sep: HSN, Naturitas y MyProtein; comprobado bueno: iHerb.
