...
- **La pauta de suplementos se hace fuera de la app** (`docs/planificacion/suplementos-pauta-externa.md`): el usuario mantiene su tabla en Drive con el skill de claude.ai `docs/planificacion/suplementos/skill/` (fuente y `suplementos.zip`) y genera `pauta-suplementos.md` con el formato de `docs/planificacion/suplementos/formato-salida.md`; la app solo la importará y la enseñará, sin seguimiento de tomas ni stock. **La tabla, la pauta y el perfil reales nunca entran en el repositorio**: son datos de salud y `docs/` es público. Si se cambia un formato, se cambia también la copia de `skill/suplementos/references/` y se vuelve a empaquetar el zip.
