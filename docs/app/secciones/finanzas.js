...
function render(cont, params) {
  ...
  cont.innerHTML = h`
    ...
    <div class="acciones"><button class="btn p" data-a="nuevo">+ Movimiento</button><label class="btn">Importar extracto <input type="file" accept=".csv,.xlsx,.xls,.pdf,text/csv,application/pdf" data-c="extracto" hidden></label><button class="btn" data-a="aplicarRec">Aplicar recurrentes (${recAplicadas}/${estado.recurrentes.length})</button>${b.n ? crudo('<button class="btn" data-a="analizarLLM">Recomendaciones con LLM</button>') : ''}</div>
    <h3>Movimientos</h3>
