...
- **El ejercicio se cuenta hablando**, como la noche en Sueño: «Contar el ejercicio» abre un cuadro de texto arriba donde se dicta lo hecho, y el mayordomo (`ejercicio-relato`) lo traduce a una lista de ejercicios, uno por línea, con series, repeticiones de cada serie o segundos si es isométrico o por tiempo, peso y descanso entre series («Dominadas: 8, 6, 5 · 10 kg · descanso 1 min 30 s»). Debajo se ve el volumen (ejercicios, series, repeticiones, tiempo, descanso y kg movidos), que se recalcula al corregir la lista. Se guarda en `sesionesEjercicio[]` con `origen: 'relato'`, la hora de inicio, el relato original y los ejercicios enlazados al catálogo cuando coinciden. Sin LLM, un lector por reglas saca del texto «N series de R», «4x12», los segundos, los kilos y el descanso. El Seguimiento resume los últimos 7 días, sumando también lo hecho en las tablas, con la duración de las sesiones y el tiempo de los ejercicios por tiempo en filas separadas.
- **Píldoras de movimiento**: durante el trabajo, cada 45/60/75 min (preferencia) un ejercicio corto. Aviso + check.

### Sueño
Objetivo: **7 h de calidad**. Modelo aceptado: un tramo largo de al menos 4,5–5 h, un despertar breve y un segundo tramo de ~2 h más ligero. **La noche se cuenta hablando**: un cuadro de texto arriba del formulario donde se dicta cómo se ha dormido, y el mayordomo (`sueno-relato`) lo traduce a hora de acostarse, minutos en dormirse, despertar nocturno, minutos despierto, hora de levantarse, calidad (1–5) y una nota, que quedan a la vista para repasarlos antes de guardar. El relato original se guarda siempre: es la fuente y los campos son su interpretación. Sin LLM, un lector por reglas saca del texto las horas que aparezcan. Cálculo de horas y racha, descontando del total tanto lo que se tarda en dormirse como los minutos despierto. Técnicas (higiene de sueño, qué hacer en el despertar nocturno) enlazadas con Meditación. Es la sección donde el mayordomo pone más acento.
...
### Finanzas
**Cuatro pestañas** (`?v=`): Movimientos (el mes, su balance y el alta manual), Importar (el módulo de carga, con el historial de extractos metidos), Informes y Recurrentes. La sección vive repartida en `docs/app/secciones/finanzas/`, y **un informe nuevo es una entrada más en el registro `INFORMES`**: por ahora solo el resumen del año; quedan por hacer la comparación entre años, la evolución de un concepto, los recurrentes detectados solos y el presupuesto frente a lo real.

Movimientos de ingreso y gasto por **concepto** (vivienda, suministros, telefonía, alimentación, restauración, suplementos, salud, transporte, compras, ropa, ocio, seguros, impuestos, servicios web, IA/LLM, hosting, efectivo, transferencias, ingresos, otros). El concepto lo adivina una tabla de patrones sacada de extractos reales, que tiene en cuenta el signo: en un abono solo valen ingresos, transferencias o efectivo, porque una devolución de Hacienda no es gasto de «impuestos». **Transferencias y efectivo son neutros**: mueven dinero sin gastarlo, y el resumen anual los descuenta para dar el gasto real. El botón **Reclasificar** vuelve a pasar las reglas sobre lo ya importado sin tocar lo que se haya editado a mano (`conceptoManual`). Importación de extractos bancarios en **CSV, Excel (.xlsx) y PDF** (mapeo de columnas, con preselección y detección de la cabecera aunque el fichero traiga preámbulo). La **descripción se compone de varias columnas** —un extracto real reparte la información entre concepto, movimiento y observaciones, y ninguna sola basta— y el **cotejo de duplicados pregunta siempre antes de escribir** si saltar o reemplazar lo que ya está, en dos niveles (fecha + importe + descripción y, con lo que sobre, fecha + importe, que son los mismos importados antes con otro mapeo) y emparejando por multiplicidad, de modo que dos apuntes gemelos del mismo día siguen siendo dos. El lector vive en `docs/app/extracto.js` y no usa ninguna librería: el `.xlsx` es un ZIP de XML y los flujos del PDF van en deflate, así que basta `DecompressionStream` + `DOMParser`. Los PDF de banco llevan fuentes Type0 (Identity-H), de modo que el texto se traduce con el CMap `/ToUnicode` de cada fuente, página a página; verificado con un extracto real de 483 movimientos de un año entero. De un PDF sin tabla reconocible se ofrece, como plan B y solo si el usuario acepta, que lo interprete el LLM (`finanzas-extracto`). Suscripciones recurrentes (servidor, OpenRouter, etc.) como gastos periódicos. Balances por mes y concepto, y **resumen del año** (`#/finanzas?v=informes&informe=anual&anio=AAAA`): una fila por mes con ingresos, gastos y neto, totales, media mensual, mes de más gasto y mejor mes, y el gasto por concepto del año entero; cada mes lleva a su detalle. Es el primer nivel del informe: filtros, comparación entre años y presupuesto vienen después. Recomendaciones del mayordomo; ejecutar acciones queda para versiones futuras.
...
| Notas | Clase (tarea, compra o idea), título, fecha tope, líneas de compra con etiqueta, etiquetas (tareas e ideas) y tipo, y si la nota junta varias cosas; una llamada por nota (y en lote para las ideas sin título y las tareas sin etiquetas) | `nota` |
